...
const AddExpense = ({ user }) => {
  ...
  const checkBudgetWarning = async () => {
    ...
    try {
      ...
      const q = query(expensesRef, where("category", "==", category), where("date", ">=", startOfMonth))
      ...
      const newTotal = currentTotal + Number.parseFloat(amount)
    ...
    <div className="container mx-auto max-w-md py-8 px-4">
      <Card className="shadow-lg">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl">{editingExpense ? "Edit Expense" : "New Expense"}</CardTitle>
          <CardDescription>
            ...
            {budgetWarning && (
              <Alert variant="warning">
                ...
                <AlertDescription className="ml-2">
                  This expense will exceed your monthly budget for {categories.find((c) => c.id === category)?.name} by
                  ₹{budgetWarning.overage.toFixed(2)}
                </AlertDescription>
              </Alert>
            )}

            <div className="flex space-x-4">
              <Button type="button" variant="outline" className="w-full h-12" onClick={() => navigate("/expenses")}>
                Cancel
              </Button>
